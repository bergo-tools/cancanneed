...
`record_session` 默认为 `false`；设为 `true` 时，pi 和 ohmypi 不再加入 `--no-session`。`timeout` 默认为每次执行 `2h`；`retries` 默认为 `1`，即首次执行失败后再重试一次，最多运行两次；`retry_backoff` 默认为 `15s`。执行超时或服务收到终止信号时，程序会终止 agent 进程组。
